...
#define BATCH_SIZE 1
    ...
    ICudaEngine *build_engine(unsigned int maxBatchSize, IBuilder *builder, DataType dt, float &gd, float &gw, std::string &wts_name)
    {
        ...
        conv10->getOutput(0)->setName(OUTPUT_BLOB_NAME);
	 ...
	 float scaleFactor1 = float(bottleneck_csp6->getOutput(0)->getDimensions().d[0]/ conv10->getOutput(0)->getDimensions().d[0]);
	 ...
	 auto upsample11 = network->addPluginV2(resizelayer1_input, 1, *resizeLayer);
        ...
        float scaleFactor2 = float(bottleneck_csp4->getOutput(0)->getDimensions().d[0]/ conv14->getOutput(0)->getDimensions().d[0]);
	mPluginAttributes.clear();
